...
def _build_prompt(state: FibrionState, module, distribution: dict, full_rejection_orders: list[str]) -> str:
    ...
    return (
        ...
        f"({len(full_rejection_orders)} total). Use this exact list and count - do not "
        f"infer this from the anomaly list above, which is sorted by a different measure "
        f"and may not include every 100%-rejection order.\n\n"
...
def run_analysis(state: FibrionState) -> dict:
    ...
    if result is None:
        ...
        return {"error": {"type": "analysis_generation_failed", "detail": meta}}
